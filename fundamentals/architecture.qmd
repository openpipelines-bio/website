...
flowchart TD  
  ingest["Ingestion"] --> Split["Split into Unimodal Objects"] --> unimodalsinglesample["Unimodal Single Sample Processing"] --> Concatenation --> unimodalmultisample["Unimodal Batch  (Multisample) Processing"] --> Merging["Merging into\n Multimodal objects"] --> dimensionality_reduction["Dimensionality Reduction \n& Cell Type Clustering"] --> integration["Integration"]  --> downstreamprocessing["Downstream Processing"]
```
...
4. [Unimodal Multisample Processing](#sec-multisample-processing): steps that require information from multiple samples together. Processing is still performed per-modality.
5. [Merging](#sec-merging): Creating one MuData object from several unimodal MuData input files. 
6. [Dimensionality Reduction](#sec-initializing-integration): Performs dimensionality reduction and cell type clustering on non-integrated samples. These are popular steps that would otherwise be executed manually or they provide input for downstream integration methods.
...
# Available workflows
The structure of the sections that have been laid out below follow a logical grouping of the processing according to the state _of the data_. However, even though this grouping makes sense from a data perspective, it does not mean that a workflow exist for each section. For example, the [processing a single sample](#sec-single-sample) section describes how processing of single sample is performed as part of the [process_samples pipeline](#sec-process-samples-pipeline), but there is no `singlesample` workflow that a user can execute. The inverse is also possible: while there exists an [process_batches](../components/workflows/multiomics/process_batches.qmd) pipeline, it's functionality is not limited to what has been described in the section [batch processing](#sec-processing-batches). This section lists all the available workflows and will try to describe the link with the relevant sections below.
...
### Processing Samples
The [process_samples](../components/workflows/multiomics/process_samples.qmd) pipeline processes files from individual samples created after ingestion into a single MuData object where samples have not yet been integrated. As will be discussed in the [ingestion](#sec-ingestion) section, which ingestion strategy you need is dependant on your technology provider and the chosen platform. For [integration](#sec-integration-methods), there exist many methods and combination of methods, and you may wish to choose which integration methods are applicable for your usecase. As a consequence, these two stages in the analysis of single-cell need to be executed seperatly and not as part of a single unified pipeline. All other steps outlined below on the other hand are included into the `process_samples` pipeline, which can therefore be summarized in the following figure:

```{mermaid}
%%| label: fig-process-samples
%%| fig-cap: Situating the process_samples workflow
flowchart TD  
  ingest["Ingestion"] --> Split["Split into Unimodal Objects"] 
  subgraph process_samples 
    Split --> unimodalsinglesample["Unimodal Single Sample Processing"] --> Concatenation --> unimodalmultisample["Unimodal Batch  (Multisample) Processing"] --> Merging["Merging into\n Multimodal objects"] --> dimensionality_reduction["Dimensionality Reduction \n& Cell Type Clustering"] 
  end
  dimensionality_reduction --> integration["Integration"]  --> downstreamprocessing["Downstream Processing"]
```


### Process Batches

Contrary to the workflows for single sample processing, there exists a [multiomics/process_batches](../components/workflows/multiomics/process_batches.qmd) workflow that performs operations on the data from multiple samples. However this workflow is not just the [multiomics/prot_multisample](../components/workflows/multiomics/prot_multisample.qmd) and [multiomics/rna_multisample](../components/workflows/multiomics/rna_multisample.qmd) workflows that have been combined. Instead, it combines the [multiomics/prot_multisample](../components/workflows/multiomics/prot_multisample.qmd), [multiomics/rna_multisample](../components/workflows/multiomics/rna_multisample.qmd) and [multiomics/integration/initialize_integration](../components/workflows/multiomics/integration/initialize_integration.qmd) workflows. 

```{mermaid}
%%| label: fig-process_batches
%%| fig-cap: Situating the process_batches pipeline .
flowchart TD  
  ingest["Ingestion"] --> Split["Split into Unimodal Objects"] --> unimodalsinglesample["Unimodal Single Sample Processing"] --> Concatenation -->unimodalmultisample 
  subgraph process_batches
    unimodalmultisample["Unimodal Batch  (Multisample) Processing"] --> Merging["Merging into\n Multimodal objects"] --> dimensionality_reduction
  end
  dimensionality_reduction["Dimensionality Reduction \n& Cell Type Clustering"] --> integration["Integration"]  --> downstreamprocessing["Downstream Processing"]
```

The purpose of this pipeline is to provide an extra 'entrypoint' into the `process_samples` pipeline that skips the singlesample processing, allowing reprocessing samples that have already been processed before. A popular usecase is to manually select one or more celltypes which need to be processed again or the integration of observations from multiple experiments into a single dataset. Keep in mind that concatenation is not included in the multisample pipeline, so when multiple input files are specified they are processed in parallel. If you would like to integrate multiple experiments, you need to first concatenate them in a seperate step:

```{.d2}
file_input_1: "Batch 1\n(multisample)" {
  shape: page
}

file_input_2: "Batch 2\n(multisample)" {
  shape: page
}
...
pipeline_out_1: "Integration Output\n(from process_samples)\n..." {
  shape: parallelogram
  style.stroke-dash: 5
}

pipeline_out_2: Integration Output\n(from process_samples)\n..." {
  shape: parallelogram
  style.stroke-dash: 5
}
...
## The QC Statistics Workflow {#sec-qc-statistics-workflow}
A [qc workflow](../components/workflows/qc/qc.qmd) has been made available that allows you to add basic QC statistics to a mudata object. Additionally, it is used by other workflows in OpenPipeline when calculating QC metrics is benificial or required. In short, the pipeline performs the following steps:
1. Flag observations with a percentage of mitochondrial genes outside of provided thresholds.
2. Add basic QC statistics to the object as described in the [calculate_qc_metrics](../components/modules/qc/calculate_qc_metrics.qmd) component.
...
multisample: "To Batch Processing"{
  shape: parallelogram
  style.stroke-dash: 5
}
...
Single-sample gene expression processing involves the following steps: calculating count-based qc statistics for unfiltered data, flagging observations with an unexpected amount of mitochondrial genes, removing cells based on count statistics and finally flagging observations originating from doublets . 
...
# Batch or Multisample Processing {#sec-processing-batches}
After the processing of individual samples has been concluded, samples can be concatenated for further processing. Like with the single-sample processing, analyses that require data across samples are not performed on multimodal objects, but each modality separately in order to tailor for the specific modality in question. This means that the result from the singlesample processing is merged together per-modality to create unimodal multisample objects. After processing each modality, all of the modalities can finally be merged and a single object is created that is ready for the integration.
...
multisample_gex: "Batch (multisample) Processing\nGene Expression" {
  shape: parallelogram
}

multisample_adt: "Batch (multisample) Processing\nAntibody Capture" {
  shape: parallelogram
}
...
4. QC metric calculations as described in [the QC statistics workflow](#sec-qc-statistics-workflow).
...
Processing the ADT modality for multiple samples is similar to the steps described for gene expression, without performing highly variable gene detection and log transformation: 

1. Normalization across samples. For ADT data, the [CLR](../components/modules/transform/clr.qmd) method is used.
2. QC metric calculations as described in [the QC statistics workflow](#sec-qc-statistics-workflow).
...
# Dimensionality Reduction and Cell Clustering {#sec-intergration}
...
## Dimensionality Reduction of Non-Integrated data {#sec-initializing-integration}
...
# Putting it all together {#sec-full-pipeline}

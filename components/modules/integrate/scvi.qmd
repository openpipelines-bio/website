...
### Links
[Source](https://github.com/openpipelines-bio/openpipeline/blob/0.10.0/./src/integrate/scvi){.btn-action .btn-sm .btn-info .btn role="button"}
...
|`--obs_categorical_covariate` |Keys in adata.obs that correspond to categorical data. These covariates can be added in addition to the batch covariate and are also treated as nuisance factors (i.e., the model tries to minimize their effects on the latent space). Thus, these should not be used for biologically-relevant factors that you do _not_ want to correct for. |`string`                         |

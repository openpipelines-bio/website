...
### Links
[Source](https://github.com/openpipelines-bio/openpipeline/blob/0.11.0/./workflows/multiomics/integration/totalvi_leiden){.btn-action .btn-sm .btn-info .btn role="button"}
...
flowchart LR
    ...
    p34(leiden)
    p36(join)
    p44(move_obsm_to_obs)
    p46(join)
    p50(mix)
    p49(filter)
    p56(umap)
    p58(join)
    p67(test_wf:run_wf:test_wf:run_wf:neighbors_leiden_umap:find_neighbors:find_neighbors_process1)
    p69(join)
    p72(filter)
    p78(test_wf:run_wf:test_wf:run_wf:neighbors_leiden_umap:leiden:leiden_process1)
    p80(join)
    p88(test_wf:run_wf:test_wf:run_wf:neighbors_leiden_umap:move_obsm_to_obs:move_obsm_to_obs_process1)
    p90(join)
    p94(mix)
    p93(filter)
    p100(test_wf:run_wf:test_wf:run_wf:neighbors_leiden_umap:umap:umap_process1)
    p102(join)
    p110(publish)
    p112(join)
    p118(Output)
